<template>
  ...
  >
    <div class="banner-slot">
      <div class="item">
        <p class="title">{{ data.todayTasks }}<i class="iconfont-sys text-success">&#xe8d5;</i></p>
        <p class="subtitle">今日任务数</p>
      </div>
      <div class="item">
        <p class="title"
...
<script setup lang="ts">
  ...
  const fetchData = async () => {
    try {
      const res = await BrowserAgentDashboardService.getAdminSummary()
      data.todayTasks = res.data.todayTasks
